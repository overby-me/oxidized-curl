...
fn main() {
                ...
                Ok(gateway) => urls_with_query
                    ...
                    .map(|u| {
                        ...
                        if l.starts_with("ipfs://") || l.starts_with("ipns://") {
                            let kind = if l.starts_with("ipfs://") {
                                "ipfs"
                            } else {
                                "ipns"
                            };
    ...
    for (url_idx, url_str) in urls_with_query.iter().enumerate() {
        ...
        {
            let altsvc_over_http = u.scheme == "https" || std::env::var("CURL_ALTSVC_HTTP").is_ok();
            if altsvc_over_http {
                    ...
                    .find(|(oh, op, _, _)| oh == &needle_host && *op == u.port)
                {
                    effective_opts
                        .connect_tos
                        .push(format!("{}:{}:{alt_host}:{alt_port}", u.host, u.port));
            ...
            Ok(mut resp) => {
                ...
                if opts.alt_svc_file.is_some() && (200..400).contains(&resp.status) {
                    ...
                    for (k, v) in &resp.headers {
                        ...
                        for entry in parse_alt_svc_header(v) {
                            let already = alt_svc_entries.iter().chain(alt_svc_learned.iter()).any(
                                |(oh, op, ah, ap)| {
                                    oh == &origin_host
                                        && *op == origin_port
                                        && ah == &entry.0
                ...
                if opts.hsts_file.is_some() && (200..300).contains(&resp.status) {
                    for (k, v) in &resp.headers {
                        ...
                        {
                            let host_no_dot = url.host.trim_end_matches('.').to_lowercase();
                            let expiry_str = format_hsts_expiry(expiry);
                            if let Some(pos) =
                                hsts_entries.iter().position(|(_, h, _)| h == &host_no_dot)
                            {
    ...
    if let Some(ref path) = opts.libcurl_file {
        ...
        if needs_slist {
            ...
            for h in &user_headers {
                ...
                c.push_str(&format!(
                    "  slist1 = curl_slist_append(slist1, \"{esc}\");\n"
                ));
            }
        }
        c.push_str("\n  hnd = curl_easy_init();\n");
        ...
        if let Some(ref u) = opts.user {
            ...
            if opts.basic_explicit {
                c.push_str("  curl_easy_setopt(hnd, CURLOPT_HTTPAUTH, (long)CURLAUTH_BASIC);\n");
            }
        }
        if needs_slist {
        ...
        if opts.data.is_some() && !opts.get {
            let data_bytes = opts.data.clone().unwrap_or_default();
...
fn c_escape_bytes(bytes: &[u8]) -> String {
    ...
    for (i, &b) in bytes.iter().enumerate() {
        match b {
            ...
            c => {
                let next_hex = bytes.get(i + 1).is_some_and(|n| n.is_ascii_hexdigit());
                if next_hex {
