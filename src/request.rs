...
pub(crate) fn build_digest_auth_nc(
    ...
    let algorithm = raw_algorithm.as_deref().unwrap_or("MD5").to_uppercase();
...
fn build_request(url: &ParsedUrl, opts: &Options) -> (Vec<u8>, Option<Vec<u8>>) {
    ...
    let request_target = if opts.proxy.is_some()
        && url.scheme == "http"
        && !opts.proxy_tunnel
        && !connect_to_tunnel
        && !proxy_is_socks_p
    ...
    let digest_header_from_state = opts.digest_challenge_state.as_deref().and_then(|chal| {
        auth_user
            .and_then(|c| {
                c.split_once(':')
...
fn execute_request_inner(
    ...
    let (mut conn, connect_response) = match connect(url, opts) {
        Ok(v) => v,
        Err(e)
            if e.contains("CONNECT tunnel failed, response 407")
                && opts.proxy_user.is_some()
                && opts.connect_proxy_digest_authorization.is_none() =>
        {
            ...
            let (tgt_host, tgt_port) =
                crate::connection::connect_to_override(&url.host, url.port, &opts.connect_tos)
...
pub(crate) fn perform(url_str: &str, opts: &Options) -> Result<Response, String> {
    ...
    loop {
            ...
            && (opts.defer_auth || (opts.ntlm && opts.ntlm_authorization.is_none()))
        {
            ...
            let digest_header_and_chal = digest_challenge.and_then(|chal| {
                creds_parsed.as_ref().and_then(|(u, p)| {
                    ...
                    build_digest_auth_nc(u, p, &chal, &method, &uri, 1).map(|h| (h, chal))
                })
            });
        ...
        let did_proxy_auth_retry =
            resp.status == 407 && opts.defer_proxy_auth && opts.proxy_user.is_some();
        if resp.status == 407
            && opts.proxy_ntlm
            && opts.proxy_ntlm_authorization.is_none()
            && opts.proxy_user.is_some()
        {
                ...
                && let Some((u, p)) = proxy_creds
            {
                ...
                if resp.status == 401 && (opts.user.is_some() || url.userinfo.is_some()) {
                        ...
                        && let Some((u, p)) = site_creds
                    {
                        ...
                        if let Some(header) = build_digest_auth_nc(&u, &p, &chal, &method, &uri, 1)
                        {
                            redirect_headers.extend_from_slice(&resp.header_bytes);
                            opts.defer_auth = false;
                            opts.digest_authorization = Some(header);
