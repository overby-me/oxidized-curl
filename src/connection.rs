...
/// Extract `user[:password]` from a proxy URL like `socks5://uz3r:p4ss@host:port`.
    ...
    match scheme {
        ProxyScheme::Socks4 | ProxyScheme::Socks4a => {
            ...
            if let Some(user) = proxy_user {
                ...
                if user.len() > 65000 {
                    return Err(
                        "too large SOCKS4 username — exceeds protocol limit".into(),
                    );
                }
            ...
            tcp.write_all(&req).map_err(|e| format!("SOCKS4 write: {e}"))?;
        ...
        ProxyScheme::Socks5 | ProxyScheme::Socks5h => {
            ...
            tcp.write_all(&greet).map_err(|e| format!("SOCKS5 greet: {e}"))?;
            ...
            let stripped_host =
                crate::url::strip_ipv6_scope(target_host).trim_end_matches('.').to_string();
            ...
            req.extend_from_slice(&target_port.to_be_bytes());
            tcp.write_all(&req).map_err(|e| format!("SOCKS5 connect: {e}"))?;
...
pub(crate) fn parse_proxy_with_scheme(
    proxy: &str,
) -> Result<(String, u16, ProxyScheme), String> {
...
pub(crate) fn connect(url: &ParsedUrl, opts: &Options) -> Result<(Connection, Vec<u8>), String> {
    ...
    let tcp = if use_tunnel {
        ...
        let build_connect = |digest_header: Option<&str>, use_basic: bool, ntlm_header: Option<&str>| -> String {
            ...
            if let Some(n) = ntlm_header {
                ...
                && let Some(ref proxy_user) = opts.proxy_user
            {
            ...
            && opts.connect_proxy_digest_authorization.is_none()
        {
            ...
            let chal = response_bytes
                ...
                .and_then(|pos| {
                    ...
                    if after_colon.len() >= 7
                        && after_colon[..7].eq_ignore_ascii_case("Digest ")
                    {
