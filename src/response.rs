...
pub(crate) struct Response {
    ...
    /// Size of proxy CONNECT response headers — counted toward
    /// `%{size_header}` even when `--suppress-connect-headers` keeps
    /// them out of `--include` / `--dump-header` output (test 1288).
    pub(crate) connect_header_size: usize,
    ...
    /// The Referer that ended up being sent on the final request after
    /// auto-referer rewriting through redirects (for `%{referer}`).
    pub(crate) final_referer: Option<String>,
