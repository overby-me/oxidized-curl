...
2. **`--proto-redir`** — test 325 (restrict redirect protocols, e.g. deny HTTP from HTTPS)
3. **`--anyauth` connection reuse** — test 338 (reuse non-authed connection)
4. **Sweep 1200-1500 range** — previous sweep hung on a test; needs targeted sub-range sweeps skipping hangers
